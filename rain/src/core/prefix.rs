...
pub fn assignments<I: Eq + Hash + Clone, K: Serialize + Clone>(prefix_assignments_list: &Vec<(Prefix, Vec<I>)>, records: &Vec<K>) -> HashMap<I, Vec<K>> {
    let mut m: HashMap<I, Vec<K>> =
        HashMap::new();
...
pub fn covering<I: Eq + Hash + Clone, K: Serialize>(prefix_assignments_list: &Vec<(Prefix, Vec<I>)>, r_id: &K) -> Vec<I> {
